...
enum Output {
    ...
    static func printReminders(_ items: [ReminderItem], format: OutputFormat) {
        switch format {
        case .human:
            if items.isEmpty { print("No reminders found."); return }
            let now = Date()
            for item in items {
        ...
        case .plain:
            for item in items {
                ...
                print([item.id, item.listName, item.isCompleted ? "1" : "0",
    ...
    static func printReminderLists(_ lists: [ReminderList], format: OutputFormat) {
        switch format {
        case .human:
            if lists.isEmpty { print("No reminder lists found."); return }
    ...
    static func printEvents(_ events: [CalendarEvent], format: OutputFormat) {
        ...
        case .human:
            ...
            for event in sorted {
                ...
                if event.isAllDay {
                    ...
                    dateStr = DateParsing.formatDisplay(event.startDate)
        ...
        case .plain:
            for event in events.sorted(by: { $0.startDate < $1.startDate }) {
                ...
                print([event.id, event.calendarTitle, isoString(event.startDate),
    ...
    static func printAuthStatus(reminders: String, calendars: String, format: OutputFormat) {
        switch format {
        case .human:
        ...
        case .plain:
            ...
            print("calendars\t\(calendars)")
        case .json:
            let payload: [String: String] = ["reminders": reminders, "calendars": calendars]
            printJSON(payload)
        }
    }
